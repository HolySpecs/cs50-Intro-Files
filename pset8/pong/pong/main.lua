...
--to determine how hard the AI is going to play
AI_DIFFICULTY = 1.0
...
function love.update(dt)
    ...
    elseif gameState == 'play' then
        ...
        if ball:collides(player1) then
            ...
            ball.x = player1.x + 5
			--increase difficulty every time ball is hit
			AI_DIFFICULTY = AI_DIFFICULTY + 0.015
        ...
        if ball.x < 0 then
			...
			--reduce AI difficulty if player 2 scores
			AI_DIFFICULTY = AI_DIFFICULTY / 3
            sounds['score']:play()
        ...
        if ball.x > VIRTUAL_WIDTH then
			...
			--increase player difficulty if player 1 scores
			AI_DIFFICULTY = AI_DIFFICULTY + 0.015
            sounds['score']:play()
    ...
    -- player 2 movement (to turn into AI)
	if (player2.y + player2.height/2) >= ball.y + ball.height/2 then
		player2.dy = -PADDLE_SPEED * (0.5 * AI_DIFFICULTY)
		--makes sure the paddle speed doesn't go insanly fast
		if player2.dy < -150 then
			player2.dy = -150
		end
