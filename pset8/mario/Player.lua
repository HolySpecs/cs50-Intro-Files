...
--check if the player is touching the flag pole
function Player:is_flag(tile)
	--check if touching flag
	if tile.id == POLE_BOTTOM or tile.id == POLE_MID or tile.id == POLE_TOP then
		return true
	else
		return false
	end
...
function Player:check_win()
	--check if intersecting with flag pole
	if (self:is_flag(self.map:tileAt(self.x, self.y))) or
		(self:is_flag(self.map:tileAt(self.x + self.width - 1, self.y))) or
		(self:is_flag(self.map:tileAt(self.x, self.y + self.width - 1))) or
		(self:is_flag(self.map:tileAt(self.x + self.width - 1, self.y + self.width - 1))) then
		return true
	else
		return false
	end
...
function Player:update(dt)
	...
	--check if touching flag pole or off the screen
	if self:check_win() or self.y >= VIRTUAL_HEIGHT then
		--restarts the game so a new map can be made
